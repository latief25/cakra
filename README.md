...
Aplikasi simulator kognitif berskala makro dengan visualisasi grafis interaktif berbasis **Vis.js Force-Directed Graph** dan arsitektur backend otomatis **API Failover**. Didesain dengan antarmuka bertema _Government Intelligence Data Center Dashboard_ premium.
...
2. **Interactive Node Network (Vis.js)**:
   ...
   - **Efek Ripple**: Saat simulasi dijalankan, node utama (`MESIN UTAMA`) akan membesar, berubah warna menjadi kuning keemasan, dan memancarkan gelombang pegas (_repulsion ripple_) ke satelit.
...
3. **Kalkulasi Animasi Telementri**:
   ...
   - **Flickering Data**: Selama kalkulasi berjalan, data _Tingkat Kepatuhan_, _Total Variabel_, _Akurasi Sistem_, dan _Status Integrasi_ di dashboard akan berkedip acak (flicker) menyerupai aktivitas pemrosesan server intensif.
...
_(File `.env` ini secara otomatis diabaikan oleh Git karena sudah didaftarkan di `.gitignore` demi keamanan)._
